...
public class EncryptionUtil2 {
	...
	final Logger logger = LoggerFactory.getLogger(EncryptionUtil2.class);
	...
	public String encrypt(String toBeEncrypt, Properties properties) {
		...
		try {
			...
			Cipher cipher = Cipher.getInstance(properties.getProperty(EncryptionConstants.CIPHER_TRANSFORMATION));
	...
	public String decrypt(String encrypted, Properties properties) {
		...
		try {
			...
			result = new String(cipher.doFinal(cipherText), CommonConstants.ENCODING_UTF8);
				...
				| BadPaddingException | IllegalBlockSizeException | InvalidKeyException | IllegalArgumentException
				| UnsupportedEncodingException ex) {
